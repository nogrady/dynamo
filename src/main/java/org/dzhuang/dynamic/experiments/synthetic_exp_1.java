...
import org.dzhuang.dynamic.preprocessing.toComparison_RDyn;
import org.dzhuang.dynamic.preprocessing.toDynaMo_RDyn;

public class synthetic_exp_1 {
	...
	public static void data_format(String Input, String o, int time_points) throws Exception{
		...
		for(int i=1;i<=nItr;i++) {
			if((new File(Input+"/graph-"+i+".txt").exists())) {
				...
				while ((line=bufferedReader.readLine()) != null) {
					...
					nodes.addAll(Arrays.asList(lines));
					pw.println(line);
				}
				bufferedReader.close();
				pw.close();
		...
		int[] node2comm=new int[nodes.size()];
		BufferedReader bufferedReader = new BufferedReader(new FileReader(Input+"/1_com_ground_truth"));
		String line="";
		PrintWriter pw2=new PrintWriter((new File(Output)).getParent()+"/com_0.txt");
		int cnt_comm=0;
		while ((line=bufferedReader.readLine()) != null) {
			String[] lines=line.split("\t");
			for(int i=0;i<lines.length;i++) {
				node2comm[Integer.parseInt(lines[i])]=cnt_comm;
			}
			cnt_comm++;
			pw2.println(line);
		}
		...
		PrintWriter pw=new PrintWriter((new File(Output)).getParent()+"/1_com_ground_truth");
		for(int i=0;i<node2comm.length;i++) {
			pw.println(node2comm[i]);
		}
		pw.close();
		
		String data_name=(new File((new File(Output)).getParent())).getName();
		toDynaMo_RDyn.run(data_name, cnt);
		toComparison_RDyn.trans2Comparisons(data_name, cnt);
		
		ModularityOptimizer_Louvain.runLouvain(data_name, cnt);
  		ModularityOptimizer_DynaMo.runDynamicModularity_syn(data_name, cnt);
		...
		pw_lo.close();
  		...
  		boolean flag=false;
  		if(!(new File("synthetic_exp_1_nmi.txt")).exists() || !(new File("synthetic_exp_1_ari.txt")).exists()) {
  			flag=true;
  		}
  		
  		PrintWriter pw_res_nmi = new PrintWriter(new FileOutputStream(new File("synthetic_exp_1_nmi.txt"), true)); 
  		PrintWriter pw_res_ari = new PrintWriter(new FileOutputStream(new File("synthetic_exp_1_ari.txt"), true)); 
